# coding: utf-8
# USAGE : python ResNet101.py --epoch 60 --modelPath ../model/
...
def getmodel():
    print('[+] loading model ResNet152...', end='', flush=True)
    model=CropModel.resnet152_finetune(NB_CLASS)
    model.cuda()
    print('Done')
    return model

def train(epochNum):
    writer=SummaryWriter('../log/'+date+'/ResNet152/') # 创建 /log/日期/ResNet152的组织形式
    ...
    for epoch in range(epochNum):
        ...
        if patience==2:
            patience=0
            model.load_state_dict(torch.load('../model/ResNet152/'+date+'_loss_best.pth')['state_dict'])
            lr=lr/10
        ...
        if  log_loss < min_loss:
            try:
                ...
                utils.snapshot('../model/ResNet152/', fileName, {
                       'epoch': epoch + 1,
