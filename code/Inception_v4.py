...
def getmodel():
    print("[+] loading model Inception_V4...", end='', flush=True)
    model=CropModel.inceptionv4_finetune(NB_CLASS)
    model.cuda()
    print('Done')
    return model

def train(epochNum):
    writer=SummaryWriter('../log/'+date+'/Inception_V4/') # 创建 /log/日期/Inception_V4的组织形式
    ...
    for epoch in range(epochNum):
        ...
        if  log_loss < min_loss:
            try:
                ...
                utils.snapshot('../model/InceptionV4/', fileName, {
                       'epoch': epoch + 1,
