...
def getmodel():
    print('[+] loading model ResNet101...', end='', flush=True)
    model=CropModel.resnet101_finetune(NB_CLASS)
    model.cuda()
    print('Done')
    return model

def train(epochNum):
    writer=SummaryWriter('../log/'+date+'/ResNet101/') # 创建 /log/日期/ResNet101的组织形式
    ...
    for epoch in range(epochNum):
        ...
        if  log_loss < min_loss:
            try:
                ...
                utils.snapshot('../model/ResNet101/', fileName, {
                       'epoch': epoch + 1,
