...
def getmodel():
    print("[+] loading model Inception_V3...", end='', flush=True)
    model=CropModel.InceptionV3Finetune(NB_CLASS)
    model.cuda()
    print('Done')
    return model

def train(epochNum):
    writer=SummaryWriter('../log/'+date+'/Inception_V3/') # 创建 /log/日期/Inception_V3的组织形式
    ...
    for epoch in range(epochNum):
        ...
        if  log_loss < min_loss:
            try:
                ...
                utils.snapshot('../model/InceptionV3/', fileName, {
                       'epoch': epoch + 1,
