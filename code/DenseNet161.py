# coding: utf-8
# USAGE : python DenseNet161.py --epoch 60 --modelPath ../model/
...
def getmodel():
    print('[+] loading model DenseNet161...', end='', flush=True)
    model=CropModel.densenet161_finetune(NB_CLASS)
    model.cuda()
    print('Done')
    return model

def train(epochNum):
    writer=SummaryWriter('../log/'+date+'/DenseNet161/') # 创建 /log/日期/DenseNet161的组织形式
    ...
    for epoch in range(epochNum):
        ...
        if patience==2: # 如果两个epoch损失都未达到设置的min_loss，则重新加载模型并将学习率降低到1/5
            patience=0
            model.load_state_dict(torch.load('../model/DenseNet161/'+date+'_loss_best.pth')['state_dict'])
            lr=lr/5
        ...
        if  log_loss < min_loss:
            try:
                ...
                utils.snapshot('../model/DenseNet161/', fileName, {
                       'epoch': epoch + 1,
