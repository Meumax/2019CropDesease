# coding: utf-8
# USAGE : python Inception_v3.py --epoch 60 --modelPath ../model/
...
IMAGE_SIZE = 299
trian_transform = CropDataset.preprocess_with_augmentation(CropDataset.normalize_05,IMAGE_SIZE)
val_transform = CropDataset.preprocess(CropDataset.normalize_05,IMAGE_SIZE)
SEED=888
...
def getmodel():
    print("[+] loading model InceptionresNet_v2...", end='', flush=True)
    model=CropModel.inceptionresnetv2_finetune(NB_CLASS)
    model.cuda()
    print('Done')
    return model

def train(epochNum):
    writer=SummaryWriter('../log/'+date+'/InceptionresNet_v2/') # 创建 /log/日期/InceptionresNet_v2的组织形式
    ...
    for epoch in range(epochNum):
        ...
        if patience==2: # 如果两个epoch损失都未达到设置的min_loss，则重新加载模型并将学习率降低到1/5
            patience=0
            model.load_state_dict(torch.load('../model/InceptionresNet_v2/'+date+'_loss_best.pth')['state_dict'])
            lr=lr/10
            print('loss has increased, lr is divided by 10, lr now is :%f'%(lr))
        ...
        if  log_loss < min_loss:
            try:
                ...
                utils.snapshot('../model/InceptionresNet_v2/', fileName, {
                       'epoch': epoch + 1,
