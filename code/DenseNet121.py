# coding: utf-8
# USAGE : python DenseNet121.py --epoch 60 --modelPath ./model/
...
def getmodel():
    print('[+] loading model DenseNet121...', end='', flush=True)
    model=CropModel.densenet121_finetune(NB_CLASS)
    model.cuda()
    print('Done')
    return model

def train(epochNum):
    writer=SummaryWriter('../log/'+date+'/DenseNet121/') # 创建 /log/日期/DenseNet121的组织形式
    ...
    for epoch in range(epochNum):
        ...
        if patience==2: # 如果两个epoch损失都未达到设置的min_loss，则重新加载模型并将学习率降低到1/5
            patience=0
            model.load_state_dict(torch.load('../model/DenseNet121/'+date+'_loss_best.pth')['state_dict'])
            lr=lr/10
